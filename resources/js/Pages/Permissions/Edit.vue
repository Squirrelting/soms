...
const updatePermission = (id: number) => {
    form.put(route("users.roles-permissions.permissions.update", { id: id }), {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            form.reset();
...
<template>
    ...
    <AuthenticatedLayout>
        <template #mobileMenuName> Edii Permissions </template>

        <div class="card shadow p-5 rounded-sm bg-white dark:bg-slate-800">
            <div class="flex justify-start ml-5">
                <span class="font-bold text-xl">EDIT PERMISSIONS</span>
            </div>
            <div>
                <div class="flex justify-start m-5">
                    <form
                        class="w-full"
                        @submit.prevent="updatePermission(props.permission.id)"
                    >
                        <div>
